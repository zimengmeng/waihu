/**
 * 弹出的日期选择框默认支持选择所有的日期。
 * 在日期input输入框中通过设置属性“showSelectMode”数值可以限制日期支持点击范围。
 * 数值说明如下：
 * 1：设置小于当天的日期可以选择
 * 2：设置小于等于当天日期可以选择
 * 3：设置仅有当天日期可以选择
 ...
 * 1：当前日期之后
 * 
 * @param compareDate 比较的日期，必须为Date类型
 * @returns {Number}
 */
function compareWithSystemDate(compareDate) {
	if(compareDate.getFullYear() < systemDate.getFullYear()) {
		return -1;
	} else if(compareDate.getFullYear() == systemDate.getFullYear()) {
		if(compareDate.getMonth() < systemDate.getMonth()) {
			return -1;
		} else if(compareDate.getMonth() == systemDate.getMonth()) {
			if(compareDate.getDate() < systemDate.getDate()) {
				return -1;
			} else if(compareDate.getDate() == systemDate.getDate()) {
				return 0;
			} else {
				return 1;
			}
	...
	DateInput.prototype = {
		...
		selectMonth: function(date) {
			...
			if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() && this.currentMonth.getMonth() == newMonth.getMonth())) {
				...
				var dayCells = "";
				var showSelectMode = this.input.attr("showSelectMode");
				for (var i = 0; i <= numDays; i++) {
					var currentDay = new Date(rangeStart.getFullYear(), rangeStart.getMonth(), rangeStart.getDate() + i, 12, 00);
					if (this.isFirstDayOfWeek(currentDay)) dayCells += "<tr>";
					if(currentDay.getMonth() == date.getMonth()) {
						var compareWithSystemDateResult = compareWithSystemDate(currentDay);
						if("1" == showSelectMode) {
							if(compareWithSystemDateResult < 0) {
								dayCells += '<td class="selectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							} else {
								dayCells += '<td class="unselectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							}
						} else if("2" == showSelectMode) {
							if(compareWithSystemDateResult < 0 || compareWithSystemDateResult == 0) {
								dayCells += '<td class="selectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							} else {
								dayCells += '<td class="unselectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							}
						} else if("3" == showSelectMode) {
							if(compareWithSystemDateResult == 0) {
								dayCells += '<td class="selectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							} else {
								dayCells += '<td class="unselectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							}
						} else if("4" == showSelectMode) {
							if(compareWithSystemDateResult == 0 || compareWithSystemDateResult > 0) {
								dayCells += '<td class="selectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							} else {
								dayCells += '<td class="unselectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							}
						} else if("5" == showSelectMode) {
							if(compareWithSystemDateResult > 0) {
								dayCells += '<td class="selectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
							} else {
								dayCells += '<td class="unselectable_day" date="' + this.dateToString(currentDay) + '">' + currentDay.getDate() + '</td>';
						...
						dayCells += '<td class="unselectable_day" date=""></td>';
					}
			...
			var inputDateStr = this.input.val();
			if(inputDateStr) {
				var inputDateStrArr = inputDateStr.split("-");
				if(inputDateStrArr.length == 3) {
					var getNewDateStr = parseInt(inputDateStrArr[0]) + "-" + parseInt(inputDateStrArr[1]) + "-" + parseInt(inputDateStrArr[2]);
					$('td[date=' + getNewDateStr + ']', this.tbody).addClass("selected");
				}
			}
		},
